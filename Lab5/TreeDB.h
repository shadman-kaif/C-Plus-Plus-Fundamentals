...
private:
   TreeNode* root;
   int probesCount;
   // Functions that are used recursivley
   DBentry* findHelper(TreeNode *node, const string &name);  
   TreeNode* findDeleteHelper(TreeNode *node, const string &name, TreeNode*& parent);
   int countHelper(TreeNode* node) const; 
   void printHelper(ostream& out, TreeNode* node) const;
   bool insertHelper(TreeNode* node, DBentry* entry);                
